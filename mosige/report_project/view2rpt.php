...
//		$item->Visible = FALSE;
	...
	//		$rs->MoveFirst(); // NOTE: no need to move position
				...
				$this->FirstRowData['count282A29'] = ewr_Conv($rs->fields('count(*)'),20);
				$this->FirstRowData['weekd'] = ewr_Conv($rs->fields('weekd'),3);
		} else { // Get next row
			$rs->MoveNext();
		}
		if (!$rs->EOF) {
			$this->starttime->setDbValue($rs->fields('starttime'));
			$this->count282A29->setDbValue($rs->fields('count(*)'));
			$this->weekd->setDbValue($rs->fields('weekd'));
			$this->Val[1] = $this->starttime->CurrentValue;
			$this->Val[2] = $this->count282A29->CurrentValue;
			$this->Val[3] = $this->weekd->CurrentValue;
		} else {
			$this->starttime->setDbValue("");
			$this->count282A29->setDbValue("");
			$this->weekd->setDbValue("");
		}
	}
	...
	function RenderRow() {
		...
		if ($this->RowType == EWR_ROWTYPE_TOTAL) { // Summary row
			...
			// count(*)
			$this->count282A29->HrefValue = "";

			// weekd
			$this->weekd->HrefValue = "";
		} else {
			...
			// count(*)
			$this->count282A29->ViewValue = $this->count282A29->CurrentValue;
			$this->count282A29->CellAttrs["class"] = ($this->RecCount % 2 <> 1) ? "ewTableAltRow" : "ewTableRow";

			// weekd
			$this->weekd->ViewValue = $this->weekd->CurrentValue;
			$this->weekd->CellAttrs["class"] = ($this->RecCount % 2 <> 1) ? "ewTableAltRow" : "ewTableRow";
		...
		if ($this->RowType == EWR_ROWTYPE_TOTAL) { // Summary row
			...
			// count(*)
			$CurrentValue = $this->count282A29->CurrentValue;
			$ViewValue = &$this->count282A29->ViewValue;
			$ViewAttrs = &$this->count282A29->ViewAttrs;
			$CellAttrs = &$this->count282A29->CellAttrs;
			$HrefValue = &$this->count282A29->HrefValue;
			$LinkAttrs = &$this->count282A29->LinkAttrs;
			$this->Cell_Rendered($this->count282A29, $CurrentValue, $ViewValue, $ViewAttrs, $CellAttrs, $HrefValue, $LinkAttrs);

			// weekd
			$CurrentValue = $this->weekd->CurrentValue;
			$ViewValue = &$this->weekd->ViewValue;
			$ViewAttrs = &$this->weekd->ViewAttrs;
			$CellAttrs = &$this->weekd->CellAttrs;
			$HrefValue = &$this->weekd->HrefValue;
			$LinkAttrs = &$this->weekd->LinkAttrs;
			$this->Cell_Rendered($this->weekd, $CurrentValue, $ViewValue, $ViewAttrs, $CellAttrs, $HrefValue, $LinkAttrs);
		}
	...
	function SetupFieldCount() {
		...
		if ($this->count282A29->Visible) $this->DtlFldCount += 1;
		if ($this->weekd->Visible) $this->DtlFldCount += 1;
	}
	...
	function GetDrillDownFilter() {
		...
		if ($opt == "1" || $opt == "2") {
			...
			$_SESSION['do_view2'] = $opt;
			$_SESSION['df_view2'] = $filter;
			$_SESSION['dl_view2'] = $sFilterList;
			...
			$opt = @$_SESSION['do_view2'];
			$filter = @$_SESSION['df_view2'];
			$sFilterList = @$_SESSION['dl_view2'];
		}
		if ($opt == "1" || $opt == "2")
			$this->DrillDown = TRUE;
		if ($opt == "1") {
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="starttime"><div class="view2_starttime"><span class="ewTableHeaderCaption"><?php echo $Page->starttime->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="starttime">
<?php if ($Page->SortUrl($Page->starttime) == "") { ?>
		<div class="ewTableHeaderBtn view2_starttime">
			<span class="ewTableHeaderCaption"><?php echo $Page->starttime->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer view2_starttime" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->starttime) ?>',0);">
			<span class="ewTableHeaderCaption"><?php echo $Page->starttime->FldCaption() ?></span>
...
<?php } ?>
<?php } ?>
<?php if ($Page->count282A29->Visible) { ?>
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="count282A29"><div class="view2_count282A29"><span class="ewTableHeaderCaption"><?php echo $Page->count282A29->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="count282A29">
<?php if ($Page->SortUrl($Page->count282A29) == "") { ?>
		<div class="ewTableHeaderBtn view2_count282A29">
			<span class="ewTableHeaderCaption"><?php echo $Page->count282A29->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer view2_count282A29" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->count282A29) ?>',0);">
			<span class="ewTableHeaderCaption"><?php echo $Page->count282A29->FldCaption() ?></span>
			<span class="ewTableHeaderSort"><?php if ($Page->count282A29->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($Page->count282A29->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span>
		</div>
<?php } ?>
	</td>
<?php } ?>
<?php } ?>
<?php if ($Page->weekd->Visible) { ?>
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="weekd"><div class="view2_weekd"><span class="ewTableHeaderCaption"><?php echo $Page->weekd->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="weekd">
<?php if ($Page->SortUrl($Page->weekd) == "") { ?>
		<div class="ewTableHeaderBtn view2_weekd">
			<span class="ewTableHeaderCaption"><?php echo $Page->weekd->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer view2_weekd" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->weekd) ?>',0);">
			<span class="ewTableHeaderCaption"><?php echo $Page->weekd->FldCaption() ?></span>
			<span class="ewTableHeaderSort"><?php if ($Page->weekd->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($Page->weekd->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span>
		</div>
<?php } ?>
	</td>
<?php } ?>
...
<span data-class="tpx<?php echo $Page->RecCount ?>_<?php echo $Page->RecCount ?>_view2_starttime"<?php echo $Page->starttime->ViewAttributes() ?>><?php echo $Page->starttime->ListViewValue() ?></span></td>
<?php } ?>
<?php if ($Page->count282A29->Visible) { ?>
		<td data-field="count282A29"<?php echo $Page->count282A29->CellAttributes() ?>>
<span data-class="tpx<?php echo $Page->RecCount ?>_<?php echo $Page->RecCount ?>_view2_count282A29"<?php echo $Page->count282A29->ViewAttributes() ?>><?php echo $Page->count282A29->ListViewValue() ?></span></td>
<?php } ?>
<?php if ($Page->weekd->Visible) { ?>
		<td data-field="weekd"<?php echo $Page->weekd->CellAttributes() ?>>
<span data-class="tpx<?php echo $Page->RecCount ?>_<?php echo $Page->RecCount ?>_view2_weekd"<?php echo $Page->weekd->ViewAttributes() ?>><?php echo $Page->weekd->ListViewValue() ?></span></td>
<?php } ?>
	</tr>
<?php
