...
<?php include_once "member_userrptinfo.php" ?>
<?php
...
$member_user_rpt = NULL; // Initialize page object first

class crmember_user_rpt extends crmember_user {
	...
	// Page object name
	var $PageObjName = 'member_user_rpt';
	...
	function __construct() {
		...
		// Table object (member_user)
		if (!isset($GLOBALS["member_user"])) {
			$GLOBALS["member_user"] = &$this;
			$GLOBALS["Table"] = &$GLOBALS["member_user"];
		}
		...
		if (!defined("EWR_TABLE_NAME"))
			define("EWR_TABLE_NAME", 'member_user', TRUE);
		...
		$this->FilterOptions->TagClassName = "ewFilterOption fmember_userrpt";
	}
...
//		$item->Visible = FALSE;
		...
		$item->Body = "<a title=\"" . ewr_HtmlEncode($ReportLanguage->Phrase("ExportToEmail", TRUE)) . "\" data-caption=\"" . ewr_HtmlEncode($ReportLanguage->Phrase("ExportToEmail", TRUE)) . "\" id=\"emf_member_user\" href=\"javascript:void(0);\" onclick=\"ewr_EmailDialogShow({lnk:'emf_member_user',hdr:ewLanguage.Phrase('ExportToEmail'),url:'$url',exportid:'$exportid',el:this});\">" . $ReportLanguage->Phrase("ExportToEmail") . "</a>";
		$item->Visible = FALSE;
		...
		$item->Body = "<button type=\"button\" class=\"btn btn-default ewSearchToggle" . $SearchToggleClass . "\" title=\"" . $ReportLanguage->Phrase("SearchBtn", TRUE) . "\" data-caption=\"" . $ReportLanguage->Phrase("SearchBtn", TRUE) . "\" data-toggle=\"button\" data-form=\"fmember_userrpt\">" . $ReportLanguage->Phrase("SearchBtn") . "</button>";
		$item->Visible = FALSE;
		...
		$item->Body = "<a class=\"ewSaveFilter\" data-form=\"fmember_userrpt\" href=\"#\">" . $ReportLanguage->Phrase("SaveCurrentFilter") . "</a>";
		$item->Visible = TRUE;
		$item = &$this->FilterOptions->Add("deletefilter");
		$item->Body = "<a class=\"ewDeleteFilter\" data-form=\"fmember_userrpt\" href=\"#\">" . $ReportLanguage->Phrase("DeleteFilter") . "</a>";
		$item->Visible = TRUE;
	...
	//		$rs->MoveFirst(); // NOTE: no need to move position
				...
				$this->FirstRowData['member_allaccount'] = ewr_Conv($rs->fields('member_allaccount'),3);
				$this->FirstRowData['member_channel'] = ewr_Conv($rs->fields('member_channel'),200);
		} else { // Get next row
			$rs->MoveNext();
		}
		if (!$rs->EOF) {
			...
			$this->member_allaccount->setDbValue($rs->fields('member_allaccount'));
			$this->member_channel->setDbValue($rs->fields('member_channel'));
			...
			$this->Val[7] = $this->rule_name->CurrentValue;
			$this->Val[8] = $this->member_points->CurrentValue;
			$this->Val[9] = $this->member_weixinid->CurrentValue;
			$this->Val[10] = $this->member_weixinname->CurrentValue;
			$this->Val[11] = $this->member_cardid->CurrentValue;
			$this->Val[12] = $this->member_password->CurrentValue;
			$this->Val[13] = $this->member_enddate->CurrentValue;
			$this->Val[14] = $this->member_birthday->CurrentValue;
			$this->Val[15] = $this->member_regtime->CurrentValue;
			$this->Val[16] = $this->member_startdate->CurrentValue;
			$this->Val[17] = $this->member_days->CurrentValue;
			$this->Val[18] = $this->member_classcount->CurrentValue;
			$this->Val[19] = $this->member_attendmax->CurrentValue;
			$this->Val[20] = $this->member_isleave->CurrentValue;
			$this->Val[21] = $this->member_leavecount->CurrentValue;
			$this->Val[22] = $this->member_leavedays->CurrentValue;
			$this->Val[23] = $this->member_intro->CurrentValue;
			$this->Val[24] = $this->member_allaccount->CurrentValue;
			$this->Val[25] = $this->member_channel->CurrentValue;
		} else {
			...
			$this->member_allaccount->setDbValue("");
			$this->member_channel->setDbValue("");
		}
	}
	...
	function RenderRow() {
		...
		if ($this->RowType == EWR_ROWTYPE_TOTAL) { // Summary row
			...
			// member_allaccount
			$this->member_allaccount->HrefValue = "";

			// member_channel
			$this->member_channel->HrefValue = "";
		} else {
			...
			// member_allaccount
			$this->member_allaccount->ViewValue = $this->member_allaccount->CurrentValue;
			$this->member_allaccount->CellAttrs["class"] = ($this->RecCount % 2 <> 1) ? "ewTableAltRow" : "ewTableRow";

			// member_channel
			$this->member_channel->ViewValue = $this->member_channel->CurrentValue;
			$this->member_channel->CellAttrs["class"] = ($this->RecCount % 2 <> 1) ? "ewTableAltRow" : "ewTableRow";
		...
		if ($this->RowType == EWR_ROWTYPE_TOTAL) { // Summary row
			...
			// member_allaccount
			$CurrentValue = $this->member_allaccount->CurrentValue;
			$ViewValue = &$this->member_allaccount->ViewValue;
			$ViewAttrs = &$this->member_allaccount->ViewAttrs;
			$CellAttrs = &$this->member_allaccount->CellAttrs;
			$HrefValue = &$this->member_allaccount->HrefValue;
			$LinkAttrs = &$this->member_allaccount->LinkAttrs;
			$this->Cell_Rendered($this->member_allaccount, $CurrentValue, $ViewValue, $ViewAttrs, $CellAttrs, $HrefValue, $LinkAttrs);

			// member_channel
			$CurrentValue = $this->member_channel->CurrentValue;
			$ViewValue = &$this->member_channel->ViewValue;
			$ViewAttrs = &$this->member_channel->ViewAttrs;
			$CellAttrs = &$this->member_channel->CellAttrs;
			$HrefValue = &$this->member_channel->HrefValue;
			$LinkAttrs = &$this->member_channel->LinkAttrs;
			$this->Cell_Rendered($this->member_channel, $CurrentValue, $ViewValue, $ViewAttrs, $CellAttrs, $HrefValue, $LinkAttrs);
		}
	...
	function SetupFieldCount() {
		...
		if ($this->member_allaccount->Visible) $this->DtlFldCount += 1;
		if ($this->member_channel->Visible) $this->DtlFldCount += 1;
	}
	...
	function GetSort() {
		...
		if (strlen(@$_GET["cmd"]) > 0) {
			...
			if ($sCmd == "resetsort") {
				...
				$this->member_allaccount->setSort("");
				$this->member_channel->setSort("");
			}
...
// Create page object
if (!isset($member_user_rpt)) $member_user_rpt = new crmember_user_rpt();
if (isset($Page)) $OldPage = $Page;
$Page = &$member_user_rpt;
...
// Create page object
var member_user_rpt = new ewr_Page("member_user_rpt");

// Page properties
member_user_rpt.PageID = "rpt"; // Page ID
var EWR_PAGE_ID = member_user_rpt.PageID;

// Extend page with Chart_Rendering function
member_user_rpt.Chart_Rendering = 
...
member_user_rpt.Chart_Rendered = 
 function(chart, chartid) { // DO NOT CHANGE THIS LINE!
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="member_id"><div class="member_user_member_id"><span class="ewTableHeaderCaption"><?php echo $Page->member_id->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="member_id">
<?php if ($Page->SortUrl($Page->member_id) == "") { ?>
		<div class="ewTableHeaderBtn member_user_member_id">
			<span class="ewTableHeaderCaption"><?php echo $Page->member_id->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer member_user_member_id" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->member_id) ?>',0);">
			<span class="ewTableHeaderCaption"><?php echo $Page->member_id->FldCaption() ?></span>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="member_name"><div class="member_user_member_name"><span class="ewTableHeaderCaption"><?php echo $Page->member_name->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="member_name">
<?php if ($Page->SortUrl($Page->member_name) == "") { ?>
		<div class="ewTableHeaderBtn member_user_member_name">
			<span class="ewTableHeaderCaption"><?php echo $Page->member_name->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer member_user_member_name" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->member_name) ?>',0);">
			<span class="ewTableHeaderCaption"><?php echo $Page->member_name->FldCaption() ?></span>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="member_sex"><div class="member_user_member_sex"><span class="ewTableHeaderCaption"><?php echo $Page->member_sex->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="member_sex">
<?php if ($Page->SortUrl($Page->member_sex) == "") { ?>
		<div class="ewTableHeaderBtn member_user_member_sex">
			<span class="ewTableHeaderCaption"><?php echo $Page->member_sex->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer member_user_member_sex" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->member_sex) ?>',0);">
			<span class="ewTableHeaderCaption"><?php echo $Page->member_sex->FldCaption() ?></span>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="member_cell"><div class="member_user_member_cell"><span class="ewTableHeaderCaption"><?php echo $Page->member_cell->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="member_cell">
<?php if ($Page->SortUrl($Page->member_cell) == "") { ?>
		<div class="ewTableHeaderBtn member_user_member_cell">
			<span class="ewTableHeaderCaption"><?php echo $Page->member_cell->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer member_user_member_cell" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->member_cell) ?>',0);">
			<span class="ewTableHeaderCaption"><?php echo $Page->member_cell->FldCaption() ?></span>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="member_email"><div class="member_user_member_email"><span class="ewTableHeaderCaption"><?php echo $Page->member_email->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="member_email">
<?php if ($Page->SortUrl($Page->member_email) == "") { ?>
		<div class="ewTableHeaderBtn member_user_member_email">
			<span class="ewTableHeaderCaption"><?php echo $Page->member_email->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer member_user_member_email" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->member_email) ?>',0);">
			<span class="ewTableHeaderCaption"><?php echo $Page->member_email->FldCaption() ?></span>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="member_level"><div class="member_user_member_level"><span class="ewTableHeaderCaption"><?php echo $Page->member_level->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="member_level">
<?php if ($Page->SortUrl($Page->member_level) == "") { ?>
		<div class="ewTableHeaderBtn member_user_member_level">
			<span class="ewTableHeaderCaption"><?php echo $Page->member_level->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer member_user_member_level" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->member_level) ?>',0);">
			<span class="ewTableHeaderCaption"><?php echo $Page->member_level->FldCaption() ?></span>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="rule_name"><div class="member_user_rule_name"><span class="ewTableHeaderCaption"><?php echo $Page->rule_name->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="rule_name">
<?php if ($Page->SortUrl($Page->rule_name) == "") { ?>
		<div class="ewTableHeaderBtn member_user_rule_name">
			<span class="ewTableHeaderCaption"><?php echo $Page->rule_name->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer member_user_rule_name" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->rule_name) ?>',0);">
			<span class="ewTableHeaderCaption"><?php echo $Page->rule_name->FldCaption() ?></span>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="member_points"><div class="member_user_member_points"><span class="ewTableHeaderCaption"><?php echo $Page->member_points->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="member_points">
<?php if ($Page->SortUrl($Page->member_points) == "") { ?>
		<div class="ewTableHeaderBtn member_user_member_points">
			<span class="ewTableHeaderCaption"><?php echo $Page->member_points->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer member_user_member_points" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->member_points) ?>',0);">
			<span class="ewTableHeaderCaption"><?php echo $Page->member_points->FldCaption() ?></span>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="member_weixinid"><div class="member_user_member_weixinid"><span class="ewTableHeaderCaption"><?php echo $Page->member_weixinid->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="member_weixinid">
<?php if ($Page->SortUrl($Page->member_weixinid) == "") { ?>
		<div class="ewTableHeaderBtn member_user_member_weixinid">
			<span class="ewTableHeaderCaption"><?php echo $Page->member_weixinid->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer member_user_member_weixinid" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->member_weixinid) ?>',0);">
			<span class="ewTableHeaderCaption"><?php echo $Page->member_weixinid->FldCaption() ?></span>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="member_weixinname"><div class="member_user_member_weixinname"><span class="ewTableHeaderCaption"><?php echo $Page->member_weixinname->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="member_weixinname">
<?php if ($Page->SortUrl($Page->member_weixinname) == "") { ?>
		<div class="ewTableHeaderBtn member_user_member_weixinname">
			<span class="ewTableHeaderCaption"><?php echo $Page->member_weixinname->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer member_user_member_weixinname" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->member_weixinname) ?>',0);">
			<span class="ewTableHeaderCaption"><?php echo $Page->member_weixinname->FldCaption() ?></span>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="member_cardid"><div class="member_user_member_cardid"><span class="ewTableHeaderCaption"><?php echo $Page->member_cardid->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="member_cardid">
<?php if ($Page->SortUrl($Page->member_cardid) == "") { ?>
		<div class="ewTableHeaderBtn member_user_member_cardid">
			<span class="ewTableHeaderCaption"><?php echo $Page->member_cardid->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer member_user_member_cardid" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->member_cardid) ?>',0);">
			<span class="ewTableHeaderCaption"><?php echo $Page->member_cardid->FldCaption() ?></span>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="member_password"><div class="member_user_member_password"><span class="ewTableHeaderCaption"><?php echo $Page->member_password->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="member_password">
<?php if ($Page->SortUrl($Page->member_password) == "") { ?>
		<div class="ewTableHeaderBtn member_user_member_password">
			<span class="ewTableHeaderCaption"><?php echo $Page->member_password->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer member_user_member_password" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->member_password) ?>',0);">
			<span class="ewTableHeaderCaption"><?php echo $Page->member_password->FldCaption() ?></span>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="member_enddate"><div class="member_user_member_enddate"><span class="ewTableHeaderCaption"><?php echo $Page->member_enddate->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="member_enddate">
<?php if ($Page->SortUrl($Page->member_enddate) == "") { ?>
		<div class="ewTableHeaderBtn member_user_member_enddate">
			<span class="ewTableHeaderCaption"><?php echo $Page->member_enddate->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer member_user_member_enddate" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->member_enddate) ?>',0);">
			<span class="ewTableHeaderCaption"><?php echo $Page->member_enddate->FldCaption() ?></span>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="member_birthday"><div class="member_user_member_birthday"><span class="ewTableHeaderCaption"><?php echo $Page->member_birthday->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="member_birthday">
<?php if ($Page->SortUrl($Page->member_birthday) == "") { ?>
		<div class="ewTableHeaderBtn member_user_member_birthday">
			<span class="ewTableHeaderCaption"><?php echo $Page->member_birthday->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer member_user_member_birthday" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->member_birthday) ?>',0);">
			<span class="ewTableHeaderCaption"><?php echo $Page->member_birthday->FldCaption() ?></span>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="member_regtime"><div class="member_user_member_regtime"><span class="ewTableHeaderCaption"><?php echo $Page->member_regtime->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="member_regtime">
<?php if ($Page->SortUrl($Page->member_regtime) == "") { ?>
		<div class="ewTableHeaderBtn member_user_member_regtime">
			<span class="ewTableHeaderCaption"><?php echo $Page->member_regtime->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer member_user_member_regtime" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->member_regtime) ?>',0);">
			<span class="ewTableHeaderCaption"><?php echo $Page->member_regtime->FldCaption() ?></span>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="member_startdate"><div class="member_user_member_startdate"><span class="ewTableHeaderCaption"><?php echo $Page->member_startdate->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="member_startdate">
<?php if ($Page->SortUrl($Page->member_startdate) == "") { ?>
		<div class="ewTableHeaderBtn member_user_member_startdate">
			<span class="ewTableHeaderCaption"><?php echo $Page->member_startdate->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer member_user_member_startdate" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->member_startdate) ?>',0);">
			<span class="ewTableHeaderCaption"><?php echo $Page->member_startdate->FldCaption() ?></span>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="member_days"><div class="member_user_member_days"><span class="ewTableHeaderCaption"><?php echo $Page->member_days->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="member_days">
<?php if ($Page->SortUrl($Page->member_days) == "") { ?>
		<div class="ewTableHeaderBtn member_user_member_days">
			<span class="ewTableHeaderCaption"><?php echo $Page->member_days->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer member_user_member_days" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->member_days) ?>',0);">
			<span class="ewTableHeaderCaption"><?php echo $Page->member_days->FldCaption() ?></span>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="member_classcount"><div class="member_user_member_classcount"><span class="ewTableHeaderCaption"><?php echo $Page->member_classcount->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="member_classcount">
<?php if ($Page->SortUrl($Page->member_classcount) == "") { ?>
		<div class="ewTableHeaderBtn member_user_member_classcount">
			<span class="ewTableHeaderCaption"><?php echo $Page->member_classcount->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer member_user_member_classcount" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->member_classcount) ?>',0);">
			<span class="ewTableHeaderCaption"><?php echo $Page->member_classcount->FldCaption() ?></span>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="member_attendmax"><div class="member_user_member_attendmax"><span class="ewTableHeaderCaption"><?php echo $Page->member_attendmax->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="member_attendmax">
<?php if ($Page->SortUrl($Page->member_attendmax) == "") { ?>
		<div class="ewTableHeaderBtn member_user_member_attendmax">
			<span class="ewTableHeaderCaption"><?php echo $Page->member_attendmax->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer member_user_member_attendmax" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->member_attendmax) ?>',0);">
			<span class="ewTableHeaderCaption"><?php echo $Page->member_attendmax->FldCaption() ?></span>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="member_isleave"><div class="member_user_member_isleave"><span class="ewTableHeaderCaption"><?php echo $Page->member_isleave->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="member_isleave">
<?php if ($Page->SortUrl($Page->member_isleave) == "") { ?>
		<div class="ewTableHeaderBtn member_user_member_isleave">
			<span class="ewTableHeaderCaption"><?php echo $Page->member_isleave->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer member_user_member_isleave" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->member_isleave) ?>',0);">
			<span class="ewTableHeaderCaption"><?php echo $Page->member_isleave->FldCaption() ?></span>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="member_leavecount"><div class="member_user_member_leavecount"><span class="ewTableHeaderCaption"><?php echo $Page->member_leavecount->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="member_leavecount">
<?php if ($Page->SortUrl($Page->member_leavecount) == "") { ?>
		<div class="ewTableHeaderBtn member_user_member_leavecount">
			<span class="ewTableHeaderCaption"><?php echo $Page->member_leavecount->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer member_user_member_leavecount" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->member_leavecount) ?>',0);">
			<span class="ewTableHeaderCaption"><?php echo $Page->member_leavecount->FldCaption() ?></span>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="member_leavedays"><div class="member_user_member_leavedays"><span class="ewTableHeaderCaption"><?php echo $Page->member_leavedays->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="member_leavedays">
<?php if ($Page->SortUrl($Page->member_leavedays) == "") { ?>
		<div class="ewTableHeaderBtn member_user_member_leavedays">
			<span class="ewTableHeaderCaption"><?php echo $Page->member_leavedays->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer member_user_member_leavedays" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->member_leavedays) ?>',0);">
			<span class="ewTableHeaderCaption"><?php echo $Page->member_leavedays->FldCaption() ?></span>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="member_intro"><div class="member_user_member_intro"><span class="ewTableHeaderCaption"><?php echo $Page->member_intro->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="member_intro">
<?php if ($Page->SortUrl($Page->member_intro) == "") { ?>
		<div class="ewTableHeaderBtn member_user_member_intro">
			<span class="ewTableHeaderCaption"><?php echo $Page->member_intro->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer member_user_member_intro" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->member_intro) ?>',0);">
			<span class="ewTableHeaderCaption"><?php echo $Page->member_intro->FldCaption() ?></span>
...
<?php } ?>
<?php } ?>
<?php if ($Page->member_allaccount->Visible) { ?>
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="member_allaccount"><div class="member_user_member_allaccount"><span class="ewTableHeaderCaption"><?php echo $Page->member_allaccount->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="member_allaccount">
<?php if ($Page->SortUrl($Page->member_allaccount) == "") { ?>
		<div class="ewTableHeaderBtn member_user_member_allaccount">
			<span class="ewTableHeaderCaption"><?php echo $Page->member_allaccount->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer member_user_member_allaccount" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->member_allaccount) ?>',0);">
			<span class="ewTableHeaderCaption"><?php echo $Page->member_allaccount->FldCaption() ?></span>
			<span class="ewTableHeaderSort"><?php if ($Page->member_allaccount->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($Page->member_allaccount->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span>
		</div>
<?php } ?>
	</td>
<?php } ?>
<?php } ?>
<?php if ($Page->member_channel->Visible) { ?>
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="member_channel"><div class="member_user_member_channel"><span class="ewTableHeaderCaption"><?php echo $Page->member_channel->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="member_channel">
<?php if ($Page->SortUrl($Page->member_channel) == "") { ?>
		<div class="ewTableHeaderBtn member_user_member_channel">
			<span class="ewTableHeaderCaption"><?php echo $Page->member_channel->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer member_user_member_channel" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->member_channel) ?>',0);">
			<span class="ewTableHeaderCaption"><?php echo $Page->member_channel->FldCaption() ?></span>
			<span class="ewTableHeaderSort"><?php if ($Page->member_channel->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($Page->member_channel->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span>
		</div>
<?php } ?>
	</td>
<?php } ?>
...
<span data-class="tpx<?php echo $Page->RecCount ?>_<?php echo $Page->RecCount ?>_member_user_member_id"<?php echo $Page->member_id->ViewAttributes() ?>><?php echo $Page->member_id->ListViewValue() ?></span></td>
<?php } ?>
<?php if ($Page->member_name->Visible) { ?>
		<td data-field="member_name"<?php echo $Page->member_name->CellAttributes() ?>>
<span data-class="tpx<?php echo $Page->RecCount ?>_<?php echo $Page->RecCount ?>_member_user_member_name"<?php echo $Page->member_name->ViewAttributes() ?>><?php echo $Page->member_name->ListViewValue() ?></span></td>
<?php } ?>
<?php if ($Page->member_sex->Visible) { ?>
		<td data-field="member_sex"<?php echo $Page->member_sex->CellAttributes() ?>>
<span data-class="tpx<?php echo $Page->RecCount ?>_<?php echo $Page->RecCount ?>_member_user_member_sex"<?php echo $Page->member_sex->ViewAttributes() ?>><?php echo $Page->member_sex->ListViewValue() ?></span></td>
<?php } ?>
<?php if ($Page->member_cell->Visible) { ?>
		<td data-field="member_cell"<?php echo $Page->member_cell->CellAttributes() ?>>
<span data-class="tpx<?php echo $Page->RecCount ?>_<?php echo $Page->RecCount ?>_member_user_member_cell"<?php echo $Page->member_cell->ViewAttributes() ?>><?php echo $Page->member_cell->ListViewValue() ?></span></td>
<?php } ?>
<?php if ($Page->member_email->Visible) { ?>
		<td data-field="member_email"<?php echo $Page->member_email->CellAttributes() ?>>
<span data-class="tpx<?php echo $Page->RecCount ?>_<?php echo $Page->RecCount ?>_member_user_member_email"<?php echo $Page->member_email->ViewAttributes() ?>><?php echo $Page->member_email->ListViewValue() ?></span></td>
<?php } ?>
<?php if ($Page->member_level->Visible) { ?>
		<td data-field="member_level"<?php echo $Page->member_level->CellAttributes() ?>>
<span data-class="tpx<?php echo $Page->RecCount ?>_<?php echo $Page->RecCount ?>_member_user_member_level"<?php echo $Page->member_level->ViewAttributes() ?>><?php echo $Page->member_level->ListViewValue() ?></span></td>
<?php } ?>
<?php if ($Page->rule_name->Visible) { ?>
		<td data-field="rule_name"<?php echo $Page->rule_name->CellAttributes() ?>>
<span data-class="tpx<?php echo $Page->RecCount ?>_<?php echo $Page->RecCount ?>_member_user_rule_name"<?php echo $Page->rule_name->ViewAttributes() ?>><?php echo $Page->rule_name->ListViewValue() ?></span></td>
<?php } ?>
<?php if ($Page->member_points->Visible) { ?>
		<td data-field="member_points"<?php echo $Page->member_points->CellAttributes() ?>>
<span data-class="tpx<?php echo $Page->RecCount ?>_<?php echo $Page->RecCount ?>_member_user_member_points"<?php echo $Page->member_points->ViewAttributes() ?>><?php echo $Page->member_points->ListViewValue() ?></span></td>
<?php } ?>
<?php if ($Page->member_weixinid->Visible) { ?>
		<td data-field="member_weixinid"<?php echo $Page->member_weixinid->CellAttributes() ?>>
<span data-class="tpx<?php echo $Page->RecCount ?>_<?php echo $Page->RecCount ?>_member_user_member_weixinid"<?php echo $Page->member_weixinid->ViewAttributes() ?>><?php echo $Page->member_weixinid->ListViewValue() ?></span></td>
<?php } ?>
<?php if ($Page->member_weixinname->Visible) { ?>
		<td data-field="member_weixinname"<?php echo $Page->member_weixinname->CellAttributes() ?>>
<span data-class="tpx<?php echo $Page->RecCount ?>_<?php echo $Page->RecCount ?>_member_user_member_weixinname"<?php echo $Page->member_weixinname->ViewAttributes() ?>><?php echo $Page->member_weixinname->ListViewValue() ?></span></td>
<?php } ?>
<?php if ($Page->member_cardid->Visible) { ?>
		<td data-field="member_cardid"<?php echo $Page->member_cardid->CellAttributes() ?>>
<span data-class="tpx<?php echo $Page->RecCount ?>_<?php echo $Page->RecCount ?>_member_user_member_cardid"<?php echo $Page->member_cardid->ViewAttributes() ?>><?php echo $Page->member_cardid->ListViewValue() ?></span></td>
<?php } ?>
<?php if ($Page->member_password->Visible) { ?>
		<td data-field="member_password"<?php echo $Page->member_password->CellAttributes() ?>>
<span data-class="tpx<?php echo $Page->RecCount ?>_<?php echo $Page->RecCount ?>_member_user_member_password"<?php echo $Page->member_password->ViewAttributes() ?>><?php echo $Page->member_password->ListViewValue() ?></span></td>
<?php } ?>
<?php if ($Page->member_enddate->Visible) { ?>
		<td data-field="member_enddate"<?php echo $Page->member_enddate->CellAttributes() ?>>
<span data-class="tpx<?php echo $Page->RecCount ?>_<?php echo $Page->RecCount ?>_member_user_member_enddate"<?php echo $Page->member_enddate->ViewAttributes() ?>><?php echo $Page->member_enddate->ListViewValue() ?></span></td>
<?php } ?>
<?php if ($Page->member_birthday->Visible) { ?>
		<td data-field="member_birthday"<?php echo $Page->member_birthday->CellAttributes() ?>>
<span data-class="tpx<?php echo $Page->RecCount ?>_<?php echo $Page->RecCount ?>_member_user_member_birthday"<?php echo $Page->member_birthday->ViewAttributes() ?>><?php echo $Page->member_birthday->ListViewValue() ?></span></td>
<?php } ?>
<?php if ($Page->member_regtime->Visible) { ?>
		<td data-field="member_regtime"<?php echo $Page->member_regtime->CellAttributes() ?>>
<span data-class="tpx<?php echo $Page->RecCount ?>_<?php echo $Page->RecCount ?>_member_user_member_regtime"<?php echo $Page->member_regtime->ViewAttributes() ?>><?php echo $Page->member_regtime->ListViewValue() ?></span></td>
<?php } ?>
<?php if ($Page->member_startdate->Visible) { ?>
		<td data-field="member_startdate"<?php echo $Page->member_startdate->CellAttributes() ?>>
<span data-class="tpx<?php echo $Page->RecCount ?>_<?php echo $Page->RecCount ?>_member_user_member_startdate"<?php echo $Page->member_startdate->ViewAttributes() ?>><?php echo $Page->member_startdate->ListViewValue() ?></span></td>
<?php } ?>
<?php if ($Page->member_days->Visible) { ?>
		<td data-field="member_days"<?php echo $Page->member_days->CellAttributes() ?>>
<span data-class="tpx<?php echo $Page->RecCount ?>_<?php echo $Page->RecCount ?>_member_user_member_days"<?php echo $Page->member_days->ViewAttributes() ?>><?php echo $Page->member_days->ListViewValue() ?></span></td>
<?php } ?>
<?php if ($Page->member_classcount->Visible) { ?>
		<td data-field="member_classcount"<?php echo $Page->member_classcount->CellAttributes() ?>>
<span data-class="tpx<?php echo $Page->RecCount ?>_<?php echo $Page->RecCount ?>_member_user_member_classcount"<?php echo $Page->member_classcount->ViewAttributes() ?>><?php echo $Page->member_classcount->ListViewValue() ?></span></td>
<?php } ?>
<?php if ($Page->member_attendmax->Visible) { ?>
		<td data-field="member_attendmax"<?php echo $Page->member_attendmax->CellAttributes() ?>>
<span data-class="tpx<?php echo $Page->RecCount ?>_<?php echo $Page->RecCount ?>_member_user_member_attendmax"<?php echo $Page->member_attendmax->ViewAttributes() ?>><?php echo $Page->member_attendmax->ListViewValue() ?></span></td>
<?php } ?>
<?php if ($Page->member_isleave->Visible) { ?>
		<td data-field="member_isleave"<?php echo $Page->member_isleave->CellAttributes() ?>>
<span data-class="tpx<?php echo $Page->RecCount ?>_<?php echo $Page->RecCount ?>_member_user_member_isleave"<?php echo $Page->member_isleave->ViewAttributes() ?>><?php echo $Page->member_isleave->ListViewValue() ?></span></td>
<?php } ?>
<?php if ($Page->member_leavecount->Visible) { ?>
		<td data-field="member_leavecount"<?php echo $Page->member_leavecount->CellAttributes() ?>>
<span data-class="tpx<?php echo $Page->RecCount ?>_<?php echo $Page->RecCount ?>_member_user_member_leavecount"<?php echo $Page->member_leavecount->ViewAttributes() ?>><?php echo $Page->member_leavecount->ListViewValue() ?></span></td>
<?php } ?>
<?php if ($Page->member_leavedays->Visible) { ?>
		<td data-field="member_leavedays"<?php echo $Page->member_leavedays->CellAttributes() ?>>
<span data-class="tpx<?php echo $Page->RecCount ?>_<?php echo $Page->RecCount ?>_member_user_member_leavedays"<?php echo $Page->member_leavedays->ViewAttributes() ?>><?php echo $Page->member_leavedays->ListViewValue() ?></span></td>
<?php } ?>
<?php if ($Page->member_intro->Visible) { ?>
		<td data-field="member_intro"<?php echo $Page->member_intro->CellAttributes() ?>>
<span data-class="tpx<?php echo $Page->RecCount ?>_<?php echo $Page->RecCount ?>_member_user_member_intro"<?php echo $Page->member_intro->ViewAttributes() ?>><?php echo $Page->member_intro->ListViewValue() ?></span></td>
<?php } ?>
<?php if ($Page->member_allaccount->Visible) { ?>
		<td data-field="member_allaccount"<?php echo $Page->member_allaccount->CellAttributes() ?>>
<span data-class="tpx<?php echo $Page->RecCount ?>_<?php echo $Page->RecCount ?>_member_user_member_allaccount"<?php echo $Page->member_allaccount->ViewAttributes() ?>><?php echo $Page->member_allaccount->ListViewValue() ?></span></td>
<?php } ?>
<?php if ($Page->member_channel->Visible) { ?>
		<td data-field="member_channel"<?php echo $Page->member_channel->CellAttributes() ?>>
<span data-class="tpx<?php echo $Page->RecCount ?>_<?php echo $Page->RecCount ?>_member_user_member_channel"<?php echo $Page->member_channel->ViewAttributes() ?>><?php echo $Page->member_channel->ListViewValue() ?></span></td>
<?php } ?>
	</tr>
<?php
...
<?php include "member_userrptpager.php" ?>
<div class="clearfix"></div>
</div>
<?php } ?>
</div>
