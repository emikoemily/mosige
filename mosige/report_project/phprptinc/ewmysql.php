...
/**
 * mysqli API for PHPMaker 12
 * (C) 2002-2015 e.World Technology Limited
 *
 * This script (ewmysqli*.php) is based on:
...
/**
 * mysqlit_driver_ADOConnection
 */

class mysqlit_driver_ADOConnection extends ADOConnection
{
	var $autoCommit = true;
	var $transOff = 0;
	var $transCnt = 0;
	...
	{
		$this->dbtype = 'mysqlit';
		$this->dataProvider = 'mysqli';
		$this->last_module_name = 'mysqlit_driver';
	}
	...
	{
		if (EW_USE_mysqliI) {
			if (!function_exists('mysqlii_real_connect')) return false;
			$this->host = $host;
			...
			$this->connectionId = @mysqlii_init();
			@mysqlii_real_connect( $this->connectionId, $this->host, $this->username, $this->password, $this->database, $this->port, $this->socket, $this->clientFlags );
			if (mysqlii_connect_errno() != 0) 
			{
			...
			{
				$this->connectionId = @mysqli_pconnect( $this->host, $this->username, $this->password, $this->clientFlags );
			}
			else
			{
	...
	function SelectDB($dbname)
	{
		...
		{
			if (EW_USE_mysqliI) {
				$result = @mysqlii_select_db( $this->connectionId, $this->database );
			} else {
			...
			if($result === false)
			{
				...
				{
					if (EW_USE_mysqliI) {
						$result = @mysqlii_query( $this->connectionId, "CREATE DATABASE IF NOT EXISTS " . $this->database );
					} else {
						$result = @$db->query( "CREATE DATABASE IF NOT EXISTS " . $this->database, $this->connectionId );
					}
	...
	function ErrorMsg()
	{
		...
		{
			if (EW_USE_mysqliI) {
				return @mysqlii_error($this->connectionId);
			} else {
	...
	function ErrorNo()
	{
		...
		{
			if (EW_USE_mysqliI) {
				return @mysqlii_errno($this->connectionId);
			} else {
	...
	{
		if (EW_USE_mysqliI) {
			return @mysqlii_affected_rows($this->connectionId);
		} else {
	...
	{
		if (EW_USE_mysqliI) {
			return @mysqlii_insert_id($this->connectionId);
		} else {
	...
	function qstr($string, $magic_quotes=false)
	{
		if (!$magic_quotes) {
			if (EW_USE_mysqliI) {
				if (function_exists('mysqlii_real_escape_string')) {
					return "'" . mysqlii_real_escape_string($this->connectionId, $string) . "'";
				}
			} else {
	...
	{
		if (EW_USE_mysqliI) {
			@mysqlii_close( $this->connectionId );
		} else {
	...
	function &do_query( $sql, $offset, $nrows, $inputarr=false )
	{
		...
		if ($inputarr && is_array($inputarr)) {
			...
			foreach ($inputarr as $arr) {
				...
				$this->query_count++;
				if (EW_USE_mysqliI) {
					$resultId = @mysqlii_query($this->connectionId, $this->sql );
				} else {
					$resultId = @$db->query( $this->sql, $this->connectionId );
				}
		...
		else
		{
				...
				$this->query_count++;
				if (EW_USE_mysqliI) {
					mysqlii_query($this->connectionId,"SET NAMES UTF8");
					$resultId = mysqlii_query($this->connectionId, $this->sql );
				} else {
		...
		if (EW_USE_mysqliI) {
			...
			{
				case ADODB_FETCH_NUM: $recordset->fetchMode = mysqliI_NUM; break;
				case ADODB_FETCH_ASSOC:$recordset->fetchMode = mysqliI_ASSOC; break;
				default:
				case ADODB_FETCH_DEFAULT:
				case ADODB_FETCH_BOTH:$recordset->fetchMode = mysqliI_BOTH; break;
			}
		} else {
			switch ($ADODB_FETCH_MODE)
			{
		...
		if (EW_USE_mysqliI) {
			$recordset->_numOfRows = @mysqlii_num_rows( $resultId );
		} else {
		...
		if (EW_USE_mysqliI) {
			$recordset->_numOfFields = @mysqlii_num_fields( $resultId );
		} else {
...
class mysqlit_driver_ResultSet
{
	var $connectionId;
	var $fields;
	var $resultId;
	...
	{
		if (EW_USE_mysqliI) {
			@mysqlii_free_result( $this->resultId );
		} else {
	...
	{
		if (EW_USE_mysqliI) {
			$this->fields = @mysqlii_fetch_array($this->resultId,$this->fetchMode);
		} else {
	...
	function _seek($row)
	{
		...
		if (EW_USE_mysqliI) {
			return @mysqlii_data_seek($this->resultId,$row);
		} else {
	...
	*/
	...
	function FetchField($fieldOffset = -1) 
	{
		if (EW_USE_mysqliI) {
			...
			// $fieldOffset not supported by mysqlii
			$fieldObject = @mysqlii_fetch_field($this->resultId);
		} else {
			if ($fieldOffset != -1) {
				$fieldObject = @mysqli_fetch_field($this->resultId, $fieldOffset);
				$fieldObject->max_length = @mysqli_field_len($this->resultId,$fieldOffset);
			}
			else
			{
				$fieldObject = @mysqli_fetch_field($this->resultId);
				$fieldObject->max_length = @mysqli_field_len($this->resultId);
			}
		}
		return $fieldObject;
	}
