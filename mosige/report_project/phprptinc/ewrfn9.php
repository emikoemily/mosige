...
function CurrentProjectID() {
	...
	return "{55EDB588-8BCE-4361-B533-47C11315EBC4}";
}
...
class crBreadcrumb {
	...
	function __construct() {
		...
		$this->Links[] = array("home", "HomePage", "cv1ctb.php", "ewHome", "", FALSE); // Home
	}
...
function ewr_QuotedValue($Value, $FldType, $DbId = 0) {
	...
	switch ($FldType) {
		...
		case EWR_DATATYPE_BOOLEAN:
			if ($dbtype == "mysqli" || $dbtype == "POSTGRESQL")
...
// FormatDateTime
// Format a timestamp, datetime, date or time field from mysqli
// $namedformat:
// 0 - General Date
// 1 - Long Date
...
function ewr_CrossTabField($smrytype, $smryfld, $colfld, $datetype, $val, $qc, $alias="", $dbid=0) {
	...
	case "MAX":
		...
		elseif ($dbtype == "mysqli" || $dbtype == "POSTGRESQL")
			$fld = $smrytype . "(IF(" . $aggwrk . "=0,NULL," . $smryfld . "))";
		break;
	case "AVG":
...
function ewr_DateVal($FldOpr, $FldVal, $ValType, $dbid = 0) {
	...
	if (ewr_GetConnectionType($dbid) <> "mysqli")
...
function ewr_GetMultiSearchSql($fn, $val, $dbid) {
	if ($val == EWR_INIT_VALUE || $val == EWR_ALL_VALUE) {
		$sSql = "";
	} elseif (ewr_GetConnectionType($dbid) == "mysqli") {
...
function ewr_GetConnectionType($dbid = 0) {
	...
	if ($db) {
		return $db["type"];
	} elseif (ewr_SameText($dbid, "mysqli")) {
		return "mysqli";
...
function ewr_ConnectDb(&$info) {
	...
	if ($dbtype == "mysqli") {
		if (EW_USE_ADODB) {
			if (EW_USE_mysqliI)
				$conn = ADONewConnection('mysqlii');
			else
				$conn = ADONewConnection('mysqlit');
		} else {
			$conn = new mysqlit_driver_ADOConnection();
		}
	} elseif ($dbtype == "POSTGRESQL") {
	...
	if ($dbtype == "mysqli" || $dbtype == "POSTGRESQL" || $dbtype == "ORACLE")
	...
	if ($dbtype == "mysqli" || $dbtype == "POSTGRESQL" || $dbtype == "ORACLE") {
		...
		if ($dbtype == "mysqli" && EWR_mysqli_CHARSET <> "")
			$conn->Execute("SET NAMES '" . EWR_mysqli_CHARSET . "'");
		if ($dbtype == "ORACLE") {
...
function ewr_Like($pat, $dbid = 0) {
	...
	if ($dbtype == "POSTGRESQL") {
		...
		if (EWR_LIKE_COLLATION_FOR_mysqli <> "") {
			return " LIKE " . $pat . " COLLATE " . EWR_LIKE_COLLATION_FOR_mysqli;
		} else {
			return " LIKE " . $pat;
		}
	} elseif ($dbtype == "MSSQL") {
...
function ewr_GroupSql($FldExpr, $GrpType, $GrpInt = 0, $dbid = 0) {
	...
	switch ($GrpType) {
		case "f": // First n characters
			...
			else if ($dbtype == "MSSQL" || $dbtype == "mysqli") // MSSQL / mysqli
		...
		case "i": // Interval
			...
			break;
		case "y": // Year
			if ($dbtype == "ACCESS" || $dbtype == "MSSQL" || $dbtype == "mysqli") // Access / MSSQL / mysqli
				return "YEAR(" . $FldExpr . ")";
			else // PostgreSql / Oracle
