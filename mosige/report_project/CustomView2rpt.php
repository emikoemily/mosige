...
<?php include_once "CustomView2rptinfo.php" ?>
<?php
...
$CustomView2_rpt = NULL; // Initialize page object first

class crCustomView2_rpt extends crCustomView2 {
	...
	function __construct() {
		...
		if (!isset($GLOBALS["CustomView2"])) {
			$GLOBALS["CustomView2"] = &$this;
			$GLOBALS["Table"] = &$GLOBALS["CustomView2"];
		}
		...
		if (!defined("EWR_TABLE_NAME"))
			define("EWR_TABLE_NAME", 'CustomView2', TRUE);
		...
		$this->FilterOptions->TagClassName = "ewFilterOption fCustomView2rpt";
	}
...
//		$item->Visible = FALSE;
		...
		$item->Body = "<a title=\"" . ewr_HtmlEncode($ReportLanguage->Phrase("ExportToEmail", TRUE)) . "\" data-caption=\"" . ewr_HtmlEncode($ReportLanguage->Phrase("ExportToEmail", TRUE)) . "\" id=\"emf_CustomView2\" href=\"javascript:void(0);\" onclick=\"ewr_EmailDialogShow({lnk:'emf_CustomView2',hdr:ewLanguage.Phrase('ExportToEmail'),url:'$url',exportid:'$exportid',el:this});\">" . $ReportLanguage->Phrase("ExportToEmail") . "</a>";
		$item->Visible = TRUE;
		...
		$item->Body = "<button type=\"button\" class=\"btn btn-default ewSearchToggle" . $SearchToggleClass . "\" title=\"" . $ReportLanguage->Phrase("SearchBtn", TRUE) . "\" data-caption=\"" . $ReportLanguage->Phrase("SearchBtn", TRUE) . "\" data-toggle=\"button\" data-form=\"fCustomView2rpt\">" . $ReportLanguage->Phrase("SearchBtn") . "</button>";
		$item->Visible = FALSE;
		...
		$item->Body = "<a class=\"ewSaveFilter\" data-form=\"fCustomView2rpt\" href=\"#\">" . $ReportLanguage->Phrase("SaveCurrentFilter") . "</a>";
		$item->Visible = TRUE;
		$item = &$this->FilterOptions->Add("deletefilter");
		$item->Body = "<a class=\"ewDeleteFilter\" data-form=\"fCustomView2rpt\" href=\"#\">" . $ReportLanguage->Phrase("DeleteFilter") . "</a>";
		$item->Visible = TRUE;
	...
	//		$rs->MoveFirst(); // NOTE: no need to move position
				...
				$this->FirstRowData['CountPerPerson'] = ewr_Conv($rs->fields('CountPerPerson'),20);
		...
		if (!$rs->EOF) {
			...
			$this->CountPerPerson->setDbValue($rs->fields('CountPerPerson'));
			$this->starttime->setDbValue($rs->fields('starttime'));
			$this->Val[1] = $this->weekday->CurrentValue;
			$this->Val[2] = $this->member_name->CurrentValue;
			$this->Val[3] = $this->CountPerPerson->CurrentValue;
		} else {
			$this->weekday->setDbValue("");
			$this->member_name->setDbValue("");
			$this->CountPerPerson->setDbValue("");
	...
	function ProcessAjaxPopup() {
		...
		if (@$_GET["popup"] <> "") {
			...
			if ($popupname == 'CustomView2_weekday') {
				$bNullValue = FALSE;
				$bEmptyValue = FALSE;
				$sFilter = $this->Filter;
			...
			if ($popupname == 'CustomView2_member_name') {
				$bNullValue = FALSE;
				$bEmptyValue = FALSE;
				$sFilter = $this->Filter;
	...
	function SetupPopup() {
		...
		if (is_array(@$_SESSION["sel_CustomView2_weekday"])) {
			$this->LoadSelectionFromSession('weekday');
		} elseif (@$_SESSION["sel_CustomView2_weekday"] == EWR_INIT_VALUE) { // Select all
			$this->weekday->SelectionList = "";
		}

		// Get member_name selected values
		if (is_array(@$_SESSION["sel_CustomView2_member_name"])) {
			$this->LoadSelectionFromSession('member_name');
		} elseif (@$_SESSION["sel_CustomView2_member_name"] == EWR_INIT_VALUE) { // Select all
			$this->member_name->SelectionList = "";
		}
	}
	...
	function RenderRow() {
		...
		if ($this->RowType == EWR_ROWTYPE_TOTAL) { // Summary row
			...
			// CountPerPerson
			$this->CountPerPerson->ViewValue = $this->CountPerPerson->CurrentValue;
			$this->CountPerPerson->CellAttrs["class"] = ($this->RecCount % 2 <> 1) ? "ewTableAltRow" : "ewTableRow";
		...
		if ($this->RowType == EWR_ROWTYPE_TOTAL) { // Summary row
			...
			// CountPerPerson
			$CurrentValue = $this->CountPerPerson->CurrentValue;
			$ViewValue = &$this->CountPerPerson->ViewValue;
			$ViewAttrs = &$this->CountPerPerson->ViewAttrs;
			$CellAttrs = &$this->CountPerPerson->CellAttrs;
			$HrefValue = &$this->CountPerPerson->HrefValue;
			$LinkAttrs = &$this->CountPerPerson->LinkAttrs;
			$this->Cell_Rendered($this->CountPerPerson, $CurrentValue, $ViewValue, $ViewAttrs, $CellAttrs, $HrefValue, $LinkAttrs);
		}
	...
	function SetupFieldCount() {
		...
		if ($this->CountPerPerson->Visible) $this->DtlFldCount += 1;
	}
	...
	function ClearSessionSelection($parm) {
		$_SESSION["sel_CustomView2_$parm"] = "";
		$_SESSION["rf_CustomView2_$parm"] = "";
		$_SESSION["rt_CustomView2_$parm"] = "";
	}

	// Load selection from session
	function LoadSelectionFromSession($parm) {
		$fld = &$this->fields($parm);
		$fld->SelectionList = @$_SESSION["sel_CustomView2_$parm"];
		$fld->RangeFrom = @$_SESSION["rf_CustomView2_$parm"];
		$fld->RangeTo = @$_SESSION["rt_CustomView2_$parm"];
	}
	...
	function RestoreFilterList() {
		...
		if (array_key_exists("sel_weekday", $filter)) {
			...
			$_SESSION["sel_CustomView2_weekday"] = $sWrk;
			$bRestoreFilter = TRUE;
		}
		...
		if (array_key_exists("sel_member_name", $filter)) {
			...
			$_SESSION["sel_CustomView2_member_name"] = $sWrk;
			$bRestoreFilter = TRUE;
		}
		if (!$bRestoreFilter) { // Clear filter
		}
	...
	function GetDrillDownFilter() {
		...
		if ($opt == "1" || $opt == "2") {
			...
			$_SESSION['do_CustomView2'] = $opt;
			$_SESSION['df_CustomView2'] = $filter;
			$_SESSION['dl_CustomView2'] = $sFilterList;
		} elseif (@$_GET["cmd"] == "resetdrilldown") { // Clear drill down
			$_SESSION[EWR_PROJECT_NAME . "_" . $this->TableVar . "_" . EWR_TABLE_MASTER_TABLE] = "";
			$_SESSION['do_CustomView2'] = "";
			$_SESSION['df_CustomView2'] = "";
			$_SESSION['dl_CustomView2'] = "";
		} else { // Restore from Session
			$opt = @$_SESSION['do_CustomView2'];
			$filter = @$_SESSION['df_CustomView2'];
			$sFilterList = @$_SESSION['dl_CustomView2'];
		}
		if ($opt == "1" || $opt == "2")
			$this->DrillDown = TRUE;
		if ($opt == "1") {
...
// Create page object
if (!isset($CustomView2_rpt)) $CustomView2_rpt = new crCustomView2_rpt();
...
// Create page object
var CustomView2_rpt = new ewr_Page("CustomView2_rpt");

// Page properties
CustomView2_rpt.PageID = "rpt"; // Page ID
var EWR_PAGE_ID = CustomView2_rpt.PageID;

// Extend page with Chart_Rendering function
CustomView2_rpt.Chart_Rendering = 
...
CustomView2_rpt.Chart_Rendered = 
 function(chart, chartid) { // DO NOT CHANGE THIS LINE!
...
// Form object
var CurrentForm = fCustomView2rpt = new ewr_Form("fCustomView2rpt");
</script>
<?php } ?>
<?php if ($Page->Export == "" && !$Page->DrillDown) { ?>
<script type="text/javascript">
...
<?php if ($Page->Export == "" && !$Page->DrillDown) { ?>
<!-- Search form (begin) -->
<form name="fCustomView2rpt" id="fCustomView2rpt" class="form-inline ewForm ewExtFilterForm" action="<?php echo ewr_CurrentPage() ?>">
<?php $SearchPanelClass = ($Page->Filter <> "") ? " in" : " in"; ?>
</form>
<script type="text/javascript">
fCustomView2rpt.Init();
fCustomView2rpt.FilterList = <?php echo $Page->GetFilterList() ?>;
</script>
<!-- Search form (end) -->
<?php } ?>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="weekday"><div class="CustomView2_weekday"><span class="ewTableHeaderCaption"><?php echo $Page->weekday->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="weekday">
<?php if ($Page->SortUrl($Page->weekday) == "") { ?>
		<div class="ewTableHeaderBtn CustomView2_weekday">
			<span class="ewTableHeaderCaption"><?php echo $Page->weekday->FldCaption() ?></span>
			<a class="ewTableHeaderPopup" title="<?php echo $ReportLanguage->Phrase("Filter"); ?>" onclick="ewr_ShowPopup.call(this, event, 'CustomView2_weekday', false, '<?php echo $Page->weekday->RangeFrom; ?>', '<?php echo $Page->weekday->RangeTo; ?>');" id="x_weekday<?php echo $Page->Cnt[0][0]; ?>"><span class="icon-filter"></span></a>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer CustomView2_weekday" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->weekday) ?>',0);">
			<span class="ewTableHeaderCaption"><?php echo $Page->weekday->FldCaption() ?></span>
...
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="member_name"><div class="CustomView2_member_name"><span class="ewTableHeaderCaption"><?php echo $Page->member_name->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="member_name">
<?php if ($Page->SortUrl($Page->member_name) == "") { ?>
		<div class="ewTableHeaderBtn CustomView2_member_name">
			<span class="ewTableHeaderCaption"><?php echo $Page->member_name->FldCaption() ?></span>
			<a class="ewTableHeaderPopup" title="<?php echo $ReportLanguage->Phrase("Filter"); ?>" onclick="ewr_ShowPopup.call(this, event, 'CustomView2_member_name', false, '<?php echo $Page->member_name->RangeFrom; ?>', '<?php echo $Page->member_name->RangeTo; ?>');" id="x_member_name<?php echo $Page->Cnt[0][0]; ?>"><span class="icon-filter"></span></a>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer CustomView2_member_name" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->member_name) ?>',0);">
			<span class="ewTableHeaderCaption"><?php echo $Page->member_name->FldCaption() ?></span>
...
<?php } ?>
<?php } ?>
<?php if ($Page->CountPerPerson->Visible) { ?>
<?php if ($Page->Export <> "" || $Page->DrillDown) { ?>
	<td data-field="CountPerPerson"><div class="CustomView2_CountPerPerson"><span class="ewTableHeaderCaption"><?php echo $Page->CountPerPerson->FldCaption() ?></span></div></td>
<?php } else { ?>
	<td data-field="CountPerPerson">
<?php if ($Page->SortUrl($Page->CountPerPerson) == "") { ?>
		<div class="ewTableHeaderBtn CustomView2_CountPerPerson">
			<span class="ewTableHeaderCaption"><?php echo $Page->CountPerPerson->FldCaption() ?></span>
		</div>
<?php } else { ?>
		<div class="ewTableHeaderBtn ewPointer CustomView2_CountPerPerson" onclick="ewr_Sort(event,'<?php echo $Page->SortUrl($Page->CountPerPerson) ?>',0);">
			<span class="ewTableHeaderCaption"><?php echo $Page->CountPerPerson->FldCaption() ?></span>
			<span class="ewTableHeaderSort"><?php if ($Page->CountPerPerson->getSort() == "ASC") { ?><span class="caret ewSortUp"></span><?php } elseif ($Page->CountPerPerson->getSort() == "DESC") { ?><span class="caret"></span><?php } ?></span>
		</div>
<?php } ?>
	</td>
<?php } ?>
...
<span data-class="tpx<?php echo $Page->RecCount ?>_<?php echo $Page->RecCount ?>_CustomView2_weekday"<?php echo $Page->weekday->ViewAttributes() ?>><?php echo $Page->weekday->ListViewValue() ?></span></td>
<?php } ?>
<?php if ($Page->member_name->Visible) { ?>
		<td data-field="member_name"<?php echo $Page->member_name->CellAttributes() ?>>
<span data-class="tpx<?php echo $Page->RecCount ?>_<?php echo $Page->RecCount ?>_CustomView2_member_name"<?php echo $Page->member_name->ViewAttributes() ?>><?php echo $Page->member_name->ListViewValue() ?></span></td>
<?php } ?>
<?php if ($Page->CountPerPerson->Visible) { ?>
		<td data-field="CountPerPerson"<?php echo $Page->CountPerPerson->CellAttributes() ?>>
<span data-class="tpx<?php echo $Page->RecCount ?>_<?php echo $Page->RecCount ?>_CustomView2_CountPerPerson"<?php echo $Page->CountPerPerson->ViewAttributes() ?>><?php echo $Page->CountPerPerson->ListViewValue() ?></span></td>
<?php } ?>
	</tr>
<?php
...
<?php include "CustomView2rptpager.php" ?>
<div class="clearfix"></div>
</div>
<?php } ?>
</div>
...
<?php } ?>
	<!-- Bottom slot -->
<a id="cht_CustomView2_Piecountbyperson"></a>
<div class="">
<div id="div_ctl_CustomView2_Piecountbyperson" class="ewChart">
<div id="div_CustomView2_Piecountbyperson" class="ewChartDiv"></div>
<!-- grid component -->
<div id="div_CustomView2_Piecountbyperson_grid" class="ewChartGrid"></div>
</div>
</div>
<?php
...
?>
<?php include_once "CustomView2_Piecountbypersonchart.php" ?>
