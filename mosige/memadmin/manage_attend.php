...
$res->setFetchMode(PDO::FETCH_ASSOC);
	...
	foreach($rows as $row) {
		...
		echo "</td>";
		//echo "<td></td>";
		echo "<td>{$row['member_cardid']}  <b><a href=http://yoga.ibreezee.com/memadmin/member_class.php?id={$row['member_id']} target='blank'>";
		if(Member::isNearOver($row['member_id']) or Member::isMemberNearOver($row['member_id'])){
			echo "<font color=#FF0000>{$row['member_name']}</font>";
		}else{
