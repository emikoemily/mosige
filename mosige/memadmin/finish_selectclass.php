...
<?php
	...
	if( $_POST ){ 
		 ...
		 $extend_paymentid=$_POST['extend_paymentid'];
		 $maxcount_set1=$_POST['maxcount_set1'];
		 $maxcount_set2=$_POST['maxcount_set2'];
		 $maxcount_set3=$_POST['maxcount_set3'];
		 $maxcount_set4=$_POST['maxcount_set4'];
		 $maxcount_set5=$_POST['maxcount_set5'];
...
<?php   
		...
		function addpayidtosub($db,$memid,$pid,$payid,$max){
			
			$sqladdsub ="INSERT INTO `yoga_lu`.`package_subscribe` (`member_id`,`package_id`,`payment_id`,`set_max`) select '{$memid}','{$pid}','{$payid}','{$max}'  from dual where not exists (select * from package_subscribe where package_id='{$pid}' AND member_id='{$memid}' AND payment_id ='{$payid}');"; 
		...
		foreach($rows as $rowviewsub) {		
			...
			echo "</tr > ";
			if(substr($rowviewsub['package_id'],0,7)=='package' and $extend_paymentid==0){
				$total1=$total1+$rowviewsub['package_price'];
				addpayidtosub($db,$mid,$rowviewsub['package_id'],$mid.$dt,0);
				createpayment($db,$mid.$dt,$mid,$dis);				
				$haspackage=1;
				
				
			}elseif(substr($rowviewsub['package_id'],0,7)=='package' and $extend_paymentid!=0){
				$total1=$total1+$rowviewsub['package_price'];
				addpayidtosub($db,$mid,$rowviewsub['package_id'],$extend_paymentid,0);				 			
				$haspackage=1;
			...
			elseif(substr($rowviewsub['package_id'],0,4)=='set1'){
				...
				addpayidtosub($db,$mid,$rowviewsub['package_id'],"set1-".$mid.$dt,$maxcount_set1);	
			...
			elseif(substr($rowviewsub['package_id'],0,4)=='set2'){
				...
				addpayidtosub($db,$mid,$rowviewsub['package_id'],"set2-".$mid.$dt,$maxcount_set2);	
				if(substr($memrule[0],0,4)=='both' or $memrule[0] ='common_count_both'){
			...
			elseif(substr($rowviewsub['package_id'],0,4)=='set3'){
				$total2=$total2+$rowviewsub['package_price'];
			    addpayidtosub($db,$mid,$rowviewsub['package_id'],"set3-".$mid.$dt,$maxcount_set3);	
		    ...
		    elseif(substr($rowviewsub['package_id'],0,4)=='set4' ){
		    	//$total2=$total2+$rowviewsub['package_price'];
		    	addpayidtosub($db,$mid,$rowviewsub['package_id'],"set4-".$mid.$dt,$maxcount_set4);
		    	createpayment($db,"set4-".$mid.$dt,$mid,$dis);
		    	updatedays($db,"set4-".$mid.$dt,365);
		    }
		    elseif(substr($rowviewsub['package_id'],0,4)=='set5' ){
		    	//$total2=$total2+$rowviewsub['package_price'];
		    	addpayidtosub($db,$mid,$rowviewsub['package_id'],"set5-".$mid.$dt,$maxcount_set5);
		    	createpayment($db,"set5-".$mid.$dt,$mid,$dis);
		    	updatedays($db,"set5-".$mid.$dt,365);
		    }
			else{
